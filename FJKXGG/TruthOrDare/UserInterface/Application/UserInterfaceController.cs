...
internal class UserInterfaceController(IUserInterfacePort userInterface, ICardPort cardPort, IGameModePort gameModePort)
{
    ...
    public void Run()
    {
        ...
        try
        {
            ...
            try
            {
                // GetAllCards may throw exception
                if (!_cardPort.GetAllCards().Any())
                    throw new SafeException("No cards found.");
    ...
    // This methods should be moved to the Domain layer into a service, but
    // it would add unnecessary complexity to the project and these are not
    // business logic related methods, so I will leave them here
